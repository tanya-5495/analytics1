...
# mean marks of m and f in both subjects
students%>% group_by(gender,college) %>% summarise(counTotal=n(),mean(marks1)
                                                   students%>%                                                   ,max(marks2))
students%>% mutate(totalMraks=marks1+1.2*marks2) %>%arrange(-totalMraks )%>% head(n=2) # to create new column based on existing column,use mutate
# total marks in decreasing order
# summarise- 
students%>% mutate(totalMraks=marks1+1.2*marks2) %>%filter(totalMraks == max(totalMraks))
# 
students%>% slice(1:5)
students%>% slice(seq(1,30,2)) # every alternte row
